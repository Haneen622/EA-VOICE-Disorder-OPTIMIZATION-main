...
# -*- coding: utf-8 -*-
"""
Optimizer runner for feature-selection + SVM hyperparameter tuning
(PSO / WOA etc.) on SVD-style TSV datasets.

This version is adapted to work with your generated PhysioNet features TSV:
- Uses label_binary (0=healthy, 1=pathology) as the target by default
- Drops non-numeric/text columns from the feature matrix (record, meta_text, label, label_binary)
- Fixes feature-dimension handling so the FS mask length matches feature columns
"""
...
# ---- IMPORTANT: define which column is the target ----
TARGET_COL = "label_binary"   # <-- binary classification (recommended)
# If you REALLY want multi-class diagnosis, change to: TARGET_COL = "label"
# But multi-class may be unstable due to rare classes.

DROP_NON_FEATURE_COLS = ["label", "label_binary", "record", "meta_text"]

X_train_full = train.drop(columns=DROP_NON_FEATURE_COLS, errors="ignore")
X_test_full = test.drop(columns=DROP_NON_FEATURE_COLS, errors="ignore")

y_train_full = train[TARGET_COL].values.tolist()
y_test_full = test[TARGET_COL].values.tolist()

feature_cols = list(X_train_full.columns)

print("The number of features %s" % (len(feature_cols)))

# Keep this to match your original code structure
...
def selector(algo, func_details, popSize, Iter, alpha):
    ...
    if algo == 0:  # PSO
        return pso.PSO(getattr(benchmarks, function_name), lb, ub, dim, popSize, Iter, fid, alpha)

    if algo == 1:  # WOA
        return woa.WOA(getattr(benchmarks, function_name), lb, ub, dim, popSize, Iter, alpha)
...
#optimizer = [PSO, MVO, GWO, MFO, CS, BAT, WOA, FFA, SSA, GA_, HHO, SCA_, JAYA_]
optimizer = [True, True]   # [PSO, WOA]
...
for value in ALPHA:
    ...
    ExportToFile = (
        f"results/WOA_PSO/svd/alpha/experiment_svd_alpha={value}_pop={PopulationSize}_iter={Iterations}_runs={NumOfRuns}_"
    ...
    for i in range(0, len(optimizer)):
        for j in range(0, len(benchmarkfunc)):
            if (optimizer[i] is True) and (benchmarkfunc[j] is True):
                ...
                for k in range(0, NumOfRuns):
                    ...
                    try:
                        ...
                        # Two hyperparameter genes at the end:
                        # last = cost gene, second-last = gamma gene
                        cost = abs(float(x2[-1] * 34.9) + 0.1)
                        gama = abs(float(x2[-2] * 0.00009999) + 0.00000001)
                        ...
                        # Ensure length matches number of features
                        x_features = x_features[:len(feature_cols)]

                        # Build selected columns
                        selectedCols = [
                            col for idx, col in enumerate(feature_cols)
                            if int(round(x_features[idx])) == 1
                        ]

                        # Safety: if no features selected, use all
                        if len(selectedCols) == 0:
                            selectedCols = list(feature_cols)
                        ...
                        print("in optimizer the num of features after FS %s" % len(selectedCols))

                        svm = SVC(C=cost, gamma=gama, kernel="rbf", random_state=42)
                        svm.fit(X_train, y_train)
                        y_pred = svm.predict(X_test)

                        # --- metrics ---
                        accuracy = accuracy_score(y_test, y_pred)
                        f1_score1 = f1_score(y_test, y_pred, average="binary" if TARGET_COL == "label_binary" else "macro")
                        recall = recall_score(y_test, y_pred, average="binary" if TARGET_COL == "label_binary" else "macro")
                        precision = precision_score(y_test, y_pred, average="binary" if TARGET_COL == "label_binary" else "macro")
                        ...
                        # For binary, keys are '0' and '1'. For multi-class, may differ.
                        if "0" in report and "1" in report:
                            ...
                            R1[0] = report["1"]["recall"]
                            F0[0] = report["0"]["f1-score"]
                            F1c[0] = report["1"]["f1-score"]
                            Gm[0] = math.sqrt(report["0"]["recall"] * report["1"]["recall"])
                        else:
                            P0[0] = P1[0] = R0[0] = R1[0] = F0[0] = F1c[0] = Gm[0] = 0.0
                        ...
                        if Export:
                            ...
                            import os
                            os.makedirs(os.path.dirname(ExportToFile), exist_ok=True)
                            ...
                            with open(ExportToFile, "a", newline="\n") as out:
                                ...
                                if not Flag:
                                    header = numpy.concatenate(
                                        [["Optimizer", "objfname", "startTime", "EndTime", "ExecutionTime", "Measure "], CnvgHeader]
                                    )
                                    writer.writerow(header)

                                a = numpy.concatenate([[x.optimizer, x.objfname, x.startTime, x.endTime, x.executionTime, "Convergence"], x.convergence])
                                b = numpy.concatenate([[x.optimizer, x.objfname, x.startTime, x.endTime, x.executionTime, "Best ACC"], ACC])
                                c = numpy.concatenate([[x.optimizer, x.objfname, x.startTime, x.endTime, x.executionTime, "Best F1"], F1s])
                                e = numpy.concatenate([[x.optimizer, x.objfname, x.startTime, x.endTime, x.executionTime, "Best Recall"], RECALL])
                                d = numpy.concatenate([[x.optimizer, x.objfname, x.startTime, x.endTime, x.executionTime, "Best Precision"], PREC])

                                tn_row = numpy.concatenate([[x.optimizer, x.objfname, x.startTime, x.endTime, x.executionTime, "TN"], TN])
                                tp_row = numpy.concatenate([[x.optimizer, x.objfname, x.startTime, x.endTime, x.executionTime, "TP"], TP])
                                fp_row = numpy.concatenate([[x.optimizer, x.objfname, x.startTime, x.endTime, x.executionTime, "FP"], FP])
                                fn_row = numpy.concatenate([[x.optimizer, x.objfname, x.startTime, x.endTime, x.executionTime, "FN"], FN])

                                p0_row = numpy.concatenate([[x.optimizer, x.objfname, x.startTime, x.endTime, x.executionTime, "P0"], P0])
                                p1_row = numpy.concatenate([[x.optimizer, x.objfname, x.startTime, x.endTime, x.executionTime, "P1"], P1])
                                r0_row = numpy.concatenate([[x.optimizer, x.objfname, x.startTime, x.endTime, x.executionTime, "R0"], R0])
                                r1_row = numpy.concatenate([[x.optimizer, x.objfname, x.startTime, x.endTime, x.executionTime, "R1"], R1])
                                f0_row = numpy.concatenate([[x.optimizer, x.objfname, x.startTime, x.endTime, x.executionTime, "F0"], F0])
                                f1_row = numpy.concatenate([[x.optimizer, x.objfname, x.startTime, x.endTime, x.executionTime, "F1"], F1c])
                                gm_row = numpy.concatenate([[x.optimizer, x.objfname, x.startTime, x.endTime, x.executionTime, "GM"], Gm])
                                fpr_row = numpy.concatenate([[x.optimizer, x.objfname, x.startTime, x.endTime, x.executionTime, "FPR"], FPR])
                                fnr_row = numpy.concatenate([[x.optimizer, x.objfname, x.startTime, x.endTime, x.executionTime, "FNR"], FNR])
                                auc_row = numpy.concatenate([[x.optimizer, x.objfname, x.startTime, x.endTime, x.executionTime, "AUC"], AUC])
                                ...
                                writer.writerow(tn_row)
                                writer.writerow(tp_row)
                                writer.writerow(fp_row)
                                writer.writerow(fn_row)

                                writer.writerow(p0_row)
                                writer.writerow(p1_row)
                                writer.writerow(r0_row)
                                writer.writerow(r1_row)
                                writer.writerow(f0_row)
                                writer.writerow(f1_row)
                                writer.writerow(gm_row)

                                writer.writerow(fpr_row)
                                writer.writerow(fnr_row)
                        ...
                        print(e)
                        print("The len of best individual is zero / or evaluation failed")
                        continue
